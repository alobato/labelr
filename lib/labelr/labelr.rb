...
module Labelr
  ...
  class Document
    ...
    def initialize(model)
      @model = model
      model_info = YAML.load_file(File.expand_path('../../../models.yml', __FILE__))[model.to_s]
  ...
  class Page
    ...
    def initialize(model)
      @model = model
      model = YAML.load_file(File.expand_path('../../../models.yml', __FILE__))[model.to_s]
      model_rows = model['rows'].to_i
      model_columns = model['columns'].to_i
      labels_by_page =  model_rows * model_columns
      @labels = Array.new(labels_by_page)
    end
